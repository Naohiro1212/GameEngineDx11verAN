...
void GameObject::SetRotateMatrix(XMMATRIX& m)
{
    transform_.matRotate_ = m;
    transform_.isSetDirect = true;
}

void GameObject::SetTranslateMatrix(XMMATRIX& m)
{
    transform_.matTranslate_ = m;
    transform_.isSetDirect = true;
}
...
void GameObject::Collision(GameObject* pTarget)
{
    ...
    // 自分とpTaretのコリジョン情報を使って当たり判定
    // 1つのオブジェクトが複数のコリジョン情報を持ってる場合もあるので二重ループ
    for (auto i = this->colliderList_.begin(); i != pTarget->colliderList_.end(); i++)
    {
